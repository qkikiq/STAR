...
from mmcv import DictAction  # 或者：from mmcv.utils import DictAction
...
class Processor():
    ...
    def __init__(self, arg):
        ...
        # # 从指定路径加载动作描述特征（使用 torch.load 读取存储的张量或字典）-- 部位描述
        self.descriptions = torch.load('/dadaY/xinyu/pycharmproject/STAR-main/semantic_feature/ntu120_part_descriptions_token.pt')
        self.action_descriptions = self.descriptions['text_embed']
    ...
    def adjust_learning_rate(self, epoch):
        if self.arg.optimizer == 'SGD' or self.arg.optimizer == 'Adam':
            if epoch < self.arg.warm_up_epoch:  # 如果当前轮次小于预热轮次，则线性增加学习率
    ...
    def train(self, epoch, save_model=False):
        ...
        loader = self.data_loader['train']  # 训练集的 DataLoader
        self.adjust_learning_rate(epoch)  # 调整学习率

        # 下面是一堆用于记录不同损失项的列表，便于统计和日志记录
        loss_value = []
        ...
        self.train_writer.add_scalar('epoch', epoch, self.global_step) # 将 epoch 写入可视化日志（如 TensorBoard）
        self.record_time() # 记录当前时间戳，用于统计耗时
        timer = dict(dataloader=0.001, model=0.001, statistics=0.001)
        process = tqdm(loader, ncols=40)

        for batch_idx, (data, label, index) in enumerate(process):  # 遍历每个 batch，返回数据、标签和索引
            self.global_step += 1
            with torch.no_grad():
                data = data.float().cuda(self.output_device)
                b,_,_,_,_ = data.size()

            timer['dataloader'] += self.split_time() # 计算数据加载耗时
            self.optimizer.zero_grad()

            # forward 前向准备阶段（构建文本/部位语义等）
            # with torch.cuda.amp.autocast():
            part_language = [] # 存放各人体部位的语言描述（例如 head/hand 等）
            # label_language = []
            for i, part_name in enumerate(["head", "hand", "arm", "hip", "leg", "foot"]): # 遍历人体部位名称
                part_language.append(self.action_descriptions[i+1].unsqueeze(1)) # 取出对应部位的描述并调整维度
                # label_language.append(self.action_descriptions[i+1])
            part_language = torch.cat(part_language, dim=1).cuda(self.output_device) # 拼接所有部位描述
            part_language_seen = part_language[seen_classes] # 仅保留已见类的部位描述
            part_language_seen_unseen = part_language[:60]  # 保留所有类别的部位描述（用于 GZSL）# ntu60-120

            prompt = torch.cat([self.model.pd_prompt[i].unsqueeze(1) for i in range(6)], dim=1)  # 拼接模型里的部位提示向量（prompt）
            part_language = torch.cat([(part_language[l.item(),:,:].unsqueeze(0)+prompt[l.item(),:,:].unsqueeze(0)) for l in label], dim=0)  # 按样本标签取对应类的部位语言并加上 prompt
            ...
            all_label_language = self.action_descriptions[0].cuda(self.output_device)[:60] # 所有 60 类的整体语义表示

            true_label_array = torch.tensor([train_label_dict[l.item()] for l in label]).cuda(self.output_device)  # 构建当前 batch 的真实标签 one-hot 编码
            all_true_label_array = torch.tensor([test_gzsl_label_dict[l.item()] for l in label]).cuda(self.output_device)


            # 前向传播：模型返回多种特征与中间结果（部位/全局视觉特征、VAE 参数、相似度、GCN 等）
            part_visual_feature, part_visual_feature_pd, global_visual_feature, part_reconstruction_embedding, \
            part_mu_feature, part_logvar_feature, sim_score, memory_weights, class_prob, label_language, part_des_mapping_feature, \
            gcn_feature, gcn_global, ske_feature, global_semantic = self.model(data, part_language, label_language,1, part_language_seen)


            # 计算各类损失：交叉熵、VAE 重构/KLD、对齐损失、提示损失、语义 CE、KL、校准、部位 CE、伪标签、未见类分类、骨架 CE、MSE 等
            loss_global_ce, loss_cvae_reconstruction, loss_cvae_kld,loss_classification, loss_mse_align, loss_align_lpd, \
            loss_align_spd, loss_prompt, loss_global_ce_semantic, loss_kl_divergence, loss_calibration_unseen, loss_part_ce,\
            loss_pd_label, loss_classification_unseen, loss_ske_ce, loss_mse_ske_lb, loss_mse_lb_pd, loss_factor, \
            loss_gcn_classify = self.model.loss_cal(part_visual_feature, global_visual_feature, part_language, part_language_seen,\
                                                    part_language_seen_unseen, label_language_seen,
            ...
            loss_value.append(loss.data.item()) # 记录总损失
            # loss_part_infonce_value.append(loss_part_infonce.data.item())
            loss_global_ce_value.append(loss_global_ce.data.item()) # 记录全局 CE 损失
            # loss_part_mse_value.append(loss_part_mse.data.item())
            loss_part_ce_value.append(loss_part_ce.data.item()) # 记录部位 CE 损失
            # loss_cali_ce_value.append(loss_cali_ce.data.item())
            # loss_global_mse_value.append(loss_global_mse.data.item())
            loss_calibration_unseen_value.append(loss_calibration_unseen.data.item()) # 记录校准损失
            loss_cvae_reconstruction_value.append(loss_cvae_reconstruction.data.item()) # 记录 VAE 重构损失
                ...
                with torch.no_grad():
                    ...
                    if ln == 'test_zsl':
                        ...
                        part_mu_feature, part_logvar_feature, sim_score, memory_weights, class_prob,sample_label_language,\
                            part_des_mapping_feature, gcn_feature, gcn_global, ske_feature, global_semantic = self.model(data, part_language, sample_label_language,0, part_language_seen)
                    ...
                    if ln == 'test_gzsl':
                        ...
                        part_visual_feature, part_visual_feature_pd, global_visual_feature, part_reconstruction_embedding, part_mu_feature, part_logvar_feature,\
                        sim_score, memory_weights, class_prob, sample_label_language,part_des_mapping_feature, gcn_feature, gcn_global, ske_feature,\
                            global_semantic = self.model(data, part_language, sample_label_language, 0, part_language_seen)
...
def get_parser():
    ...
    # processor  运行模式相关参数
    parser.add_argument('--phase', default='train', help='must be train or test') #运行模式，训练或测试
    ...
    parser.add_argument('--log-interval', type=int, default=100, help='the interval for printing messages (#iteration)') #打印日志间隔
    parser.add_argument('--save-interval', type=int, default=1, help='the interval for storing models (#iteration)') #保存模型间隔
    parser.add_argument('--save-epoch', type=int, default=30, help='the start epoch to save model (#iteration)')
    parser.add_argument('--eval-interval', type=int, default=5, help='the interval for evaluating models (#iteration)') #评估间隔
    ...
    parser.add_argument('--rgb_weights', default=None, help='the weights for network initialization') #rgb分支的权重
