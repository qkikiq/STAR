...
class ModelMatch(nn.Module):
    def __init__(self,num_class=60, num_point=25, num_person=2, graph=None, graph_args=dict(), in_channels=3,
        ...
        # body part spatial temporal attention network  空间-时间注意力网络
        ...
        self.part_weights_factor = nn.Parameter(torch.ones(1, 6), requires_grad=False) # 部位权重
        ...
        for _ in range(self.body_part):
            ...
            self.body_part_st_attention_networks.append(nn.MultiheadAttention(embed_dim=768, kdim=256, vdim=256, num_heads=4, batch_first=True))  # head:4 多头注意力
            ...
            self.body_part_mapping_matrix1.append(nn.Linear(768, 768))  # 部位描述映射1
            self.body_part_mapping_matrix2.append(nn.Linear(768, 768)) # 部位描述映射2
            ...
            self.pd_prompt.append(nn.Parameter(nn.init.normal_(torch.empty(120, 768)), requires_grad=True))  #可学习参数 可改
        self.relu = nn.ReLU()
        ...
        self.global_fc2 = nn.Linear(256, 100) # 全局特征层2
        # gcn 
        self.gcn_flobal_classifier = nn.Linear(256, 60) # GCN全局分类器
    ...
    def forward(self, x, st_attributes, part_des_feature, label_language, train_flag, part_language_seen):
        ...
        n,c,t,v = gcn_x.size()
        gcn_global = gcn_x.mean(3).mean(2)  # 全局GCN特征  ?直接使用？？？
